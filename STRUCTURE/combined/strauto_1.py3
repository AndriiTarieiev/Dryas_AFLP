...
def kfoldersf():
  mystr = ''.join("mkdir k{}\n".format(x) for x in range(1, maxpops+1))
  return mystr

## Function to print names of folders to be moved around through shell script.
def mvfolders():
  mvfold = ''.join("k{} ".format(y) for y in range(1, maxpops+1))
  return mvfold 
...
if parallel:
    ...
    for myK in range(1, maxpops+1):
        for run in range(1, kruns+1):
            structureCommands.write("%sstructure -D %d -K %d -m mainparams -o k%d/%s_k%d_run%d 2>&1 | tee log/k%d/%s_k%d_run%d.log\n" %
...
if harvest:
    harvesterInPath = cmd_exists('structureHarvester')
    if not harvesterInPath:
        if not os.path.isfile('harvester'):
            print('structureHarvester is not in the $PATH or in the current working directory. Scripts will be created')
            print('referencing this executable in the working directory. You will need to do have this')
            print('executable available in order to run any script created.\n\n')
        harprefix = "./"
    else:
    ...
    runstr.write("mkdir harvester_data\n")
    runstr.write("cp results_f/k*/*_f harvester_data\n")
    runstr.write("%sstructureHarvester --dir harvester_data --out harvester --evanno --clumpp\n" % harprefix)
    runstr.write("rm -rf harvester_data\n")
    runstr.write("echo 'structureHarvester run has finished.'\n")

## Close 'runstructure' script and assign rwx permissions to it                
runstr.close()
os.chmod("runstructure", 0o755)
